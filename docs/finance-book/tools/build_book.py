...
CONCEPT_FIGURES_FREE = {
    14: [("第一阶段", _fig_b1_roadmap)],
    22: [("保证金和现金", _fig_b1_neutral_ledger)],
    25: [("真实财富和生产力", _fig_b1_ten_layers)],
    26: [("指数增强回测", _fig_b1_portfolio)],
    27: [("货币", _fig_b1_glossary)],
    28: [("总资产不是可花的钱", _fig_b1_account_fields)],
    29: [("数据没有按时到", _fig_b2_week)],
    30: [("先定资金上限", _fig_b2_guards)],
    31: [("能解释钱和财富的区别", _fig_b2_checklist)],
    32: [("他先找模型", _fig_b2_failure)],
    33: [("问题很小", _fig_b2_pipeline)],
    34: [("问题是否足够小", _fig_b2_questions)],
}
...
PATCH_C = {
    ...
    8: [("券商是通道", _fig_c6_order_pipeline)],
    9: [("影线只是压缩图", _fig_c7_ohlc_aggregation)],
    10: [("这条路颠不颠", _fig_c8_volatility_cluster),
        ("每承担一份波动赚多少超额收益", _fig_c8_sharpe_compare)],
    11: [("高分组是不是真的更好", _fig_c9_factor_groups)],
}


PATCH_D = {
    12: [("历史答案背得太熟", _fig_d10_split)],
    13: [("预测分数不是仓位", _fig_d11_pipeline)],
    15: [("指数收益加超额", _fig_d13_alpha_beta)],
    16: [("Beta 对冲的直觉", _fig_d14_hedge_vol)],
    17: [("配对交易", _fig_d15_pair_trading)],
    6: [("三张报表分别回答什么", _fig_d17_three_tables)],
}
...
CONCEPT_FIGURES = {
    ...
    8: [("价格优先和时间优先", _fig_orderbook)],
    9: [("一天被压成四个价格", _fig_candlestick)],
    10: [("从高点跌到低点有多痛", _fig_drawdown_sharpe)],
    11: [("常见因子家族", _fig_factor_quantile)],
    12: [("收盘价成交最容易骗人", _fig_lookahead)],
    13: [("约束优化的直觉", _fig_quant_pipeline)],
    15: [("指数增强", _fig_index_enhance)],
    16: [("市场中性想解决什么", _fig_market_neutral)],
    17: [("配对交易", _fig_arbitrage)],
    18: [("净值和收益披露", _fig_fund_nav)],
    6: [("三张报表分别回答什么", _fig_income_statement)],
    20: [("目录结构先分层", _fig_project_layout)],
    21: [("模拟调仓", _fig_backtest_loop)],
    23: [("过拟合", _fig_overfitting)],
    24: [("回撤里最容易做错决定", _fig_loss_recovery)],
}
...
PATCH_E = {
    ...
    12: [("问题定义", _fig_e10_ml_shutter)],
    15: [("超额收益从哪里", _fig_e13_alpha_five)],
}
# -*- coding: utf-8 -*-
...
PATCH_F = {
    14: [("研究员每天在做什么", _fig_f12_researcher_loop)],
    18: [("业绩归因报告怎么看", _fig_f16_attribution_bar)],
    20: [("最大回撤", _fig_f18_max_drawdown)],
    21: [("让时间线先立正", _fig_f19_one_day_timeline)],
    22: [("把总收益拆回零件", _fig_f20_neutral_decompose)],
    23: [("手续费不是小零头", _fig_f21_cost_erosion)],
    24: [("复盘顺序", _fig_f22_strip_curve)],
}
# -*- coding: utf-8 -*-
...
PATCH_G = {
    25: [("回测和验证", _fig_g23_backtest_audit)],
    26: [("写出第一份可信的回测报告", _fig_g24_report_lines)],
    27: [("风控", _fig_g25_risk_toolbox)],
    28: [("看风险指标", _fig_g26_risk_metrics)],
    29: [("看收益和回撤", _fig_g27_two_funds)],
    30: [("小实盘结束后看什么", _fig_g28_graduation_matrix)],
    31: [("成本必须做敏感性", _fig_g29_cost_stress)],
    32: [("高胜率策略一定好吗", _fig_g30_winrate_trap)],
    33: [("单因子报告", _fig_g31_factor_files)],
    34: [("实盘前先问能否停止", _fig_g32_stop_rules)],
}
