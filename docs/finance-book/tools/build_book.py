...
CHAPTER_GROUPS = [
    ...
    [70], [37, 38, 65], [66], [67], [68], [69], [71], [72], [73],
...
CONCEPT_FIGURES_FREE = {
    21: [("先把金融地图补齐", _fig_b1_roadmap)],
    28: [("中性组合也会缺钱", _fig_b1_neutral_ledger)],
    31: [("真实财富和生产力", _fig_b1_ten_layers)],
    32: [("指数增强回测", _fig_b1_portfolio)],
    33: [("货币", _fig_b1_glossary)],
    34: [("总资产不是可花的钱", _fig_b1_account_fields)],
    35: [("数据没有按时到", _fig_b2_week)],
    36: [("先定资金上限", _fig_b2_guards)],
    37: [("能解释钱和财富的区别", _fig_b2_checklist)],
    38: [("他先找模型", _fig_b2_failure)],
    39: [("问题很小", _fig_b2_pipeline)],
    40: [("问题是否足够小", _fig_b2_questions)],
}
...
PATCH_C = {
    ...
    15: [("券商是通道", _fig_c6_order_pipeline)],
    16: [("影线只是压缩图", _fig_c7_ohlc_aggregation)],
    17: [("这条路颠不颠", _fig_c8_volatility_cluster),
        ("每承担一份波动赚多少超额收益", _fig_c8_sharpe_compare)],
    18: [("高分组是不是真的更好", _fig_c9_factor_groups)],
}


PATCH_D = {
    19: [("历史答案背得太熟", _fig_d10_split)],
    20: [("预测分数不是仓位", _fig_d11_pipeline)],
    22: [("指数收益加超额", _fig_d13_alpha_beta)],
    23: [("Beta 对冲的直觉", _fig_d14_hedge_vol)],
    24: [("配对交易", _fig_d15_pair_trading)],
...
CONCEPT_FIGURES = {
    ...
    15: [("价格优先和时间优先", _fig_orderbook)],
    16: [("一天被压成四个价格", _fig_candlestick)],
    17: [("从高点跌到低点有多痛", _fig_drawdown_sharpe)],
    18: [("常见因子家族", _fig_factor_quantile)],
    19: [("收盘价成交最容易骗人", _fig_lookahead)],
    20: [("约束优化的直觉", _fig_quant_pipeline)],
    22: [("指数增强", _fig_index_enhance)],
    23: [("市场中性想解决什么", _fig_market_neutral)],
    24: [("配对交易", _fig_arbitrage)],
    25: [("净值和收益披露", _fig_fund_nav)],
    7: [("三张报表分别回答什么", _fig_income_statement)],
    26: [("目录结构先分层", _fig_project_layout)],
    27: [("纸面组合落地的第一道关", _fig_backtest_loop)],
    29: [("把历史噪声背成了规律", _fig_overfitting)],
    30: [("回撤里最容易做错决定", _fig_loss_recovery)],
}
...
PATCH_E = {
    ...
    19: [("问题定义", _fig_e10_ml_shutter)],
    22: [("超额收益从哪里", _fig_e13_alpha_five)],
}
# -*- coding: utf-8 -*-
...
PATCH_F = {
    21: [("研究员每天在做什么", _fig_f12_researcher_loop)],
    25: [("业绩归因报告怎么看", _fig_f16_attribution_bar)],
    26: [("最大回撤", _fig_f18_max_drawdown)],
    27: [("让时间线先立正", _fig_f19_one_day_timeline)],
    28: [("把总收益拆回零件", _fig_f20_neutral_decompose)],
    29: [("手续费不是小零头", _fig_f21_cost_erosion)],
    30: [("复盘顺序", _fig_f22_strip_curve)],
}
# -*- coding: utf-8 -*-
...
PATCH_G = {
    31: [("回测和验证", _fig_g23_backtest_audit)],
    32: [("写出第一份可信的回测报告", _fig_g24_report_lines)],
    33: [("风控", _fig_g25_risk_toolbox)],
    34: [("看风险指标", _fig_g26_risk_metrics)],
    35: [("看收益和回撤", _fig_g27_two_funds)],
    36: [("小实盘结束后看什么", _fig_g28_graduation_matrix)],
    37: [("成本必须做敏感性", _fig_g29_cost_stress)],
    38: [("高胜率策略一定好吗", _fig_g30_winrate_trap)],
    39: [("单因子报告", _fig_g31_factor_files)],
    40: [("实盘前先问能否停止", _fig_g32_stop_rules)],
}
...
def _fig_o14_thrift():
    """ch14:节俭悖论的循环:省钱→收入下降→更省钱。"""
    body = """            <text x="450" y="34" text-anchor="middle" fill="#eef4f8" font-size="19" font-weight="800">节俭悖论:一个人的美德,一群人的灾难</text>
            <!-- 上:个人视角(正确) -->
            <rect x="60" y="70" width="340" height="110" rx="10" fill="rgba(122,167,240,0.2)" stroke="#7aa7f0"/>
            <text x="230" y="98" text-anchor="middle" fill="#a8c6ff" font-size="15" font-weight="800">个人:多存钱</text>
            <text x="230" y="124" text-anchor="middle" fill="#bcc9dd" font-size="12">钱包更厚,抗风险变强</text>
            <text x="230" y="146" text-anchor="middle" fill="#bcc9dd" font-size="12">没人受伤</text>
            <text x="230" y="170" text-anchor="middle" fill="#8fb37a" font-size="13" font-weight="800">✓ 明智</text>
            <!-- 下:全社会(灾难) -->
            <rect x="500" y="70" width="340" height="110" rx="10" fill="rgba(238,136,119,0.15)" stroke="#ee8877"/>
            <text x="670" y="98" text-anchor="middle" fill="#ffb4b4" font-size="15" font-weight="800">全社会:同时多存钱</text>
            <text x="670" y="124" text-anchor="middle" fill="#bcc9dd" font-size="12">商家收入暴跌 → 裁员降薪</text>
            <text x="670" y="146" text-anchor="middle" fill="#bcc9dd" font-size="12">人们收入下降 → 更不敢花</text>
            <text x="670" y="170" text-anchor="middle" fill="#ee8877" font-size="13" font-weight="800">✗ 通缩螺旋</text>
            <!-- 循环箭头 -->
            <path d="M 330 200 Q 450 260 570 200" fill="none" stroke="#ee8877" stroke-width="3" stroke-dasharray="8 6" marker-end="url(#arrowO)"/>
            <text x="450" y="252" text-anchor="middle" fill="#8499bd" font-size="12.5">一个人的支出就是另一个人的收入,大家都省 = 大家都没得赚</text>
            <!-- 底:历史数据 -->
            <rect x="60" y="285" width="780" height="95" rx="10" fill="rgba(18,29,49,0.7)" stroke="rgba(240,201,106,0.45)"/>
            <text x="84" y="313" fill="#f0c96a" font-size="13" font-weight="800">1929-1933 美国大萧条实录</text>
            <text x="84" y="338" fill="#bcc9dd" font-size="12">储蓄率翻倍 + 货币流通速度塌缩:GDP 四年跌近三分之一,物价缩水 24%,失业率冲到 25%</text>
            <text x="84" y="362" fill="#bcc9dd" font-size="12">破局靠财政政策:政府发债把私人储蓄接出来花掉,重新点燃“支出→收入”的链条</text>
            <defs><marker id="arrowO" markerWidth="9" markerHeight="9" refX="8" refY="4.5" orient="auto"><path d="M0,0 L9,4.5 L0,9 Z" fill="#ee8877"/></marker></defs>
            <text x="450" y="412" text-anchor="middle" fill="#8499bd" font-size="12.5">判断任何经济决策,先问:是一个人做,还是所有人一起做?</text>"""
    return concept_figure(body, "0 0 900 440",
        "节俭悖论对照图:左半个人多存钱是明智,右半全社会同步存钱导致通缩螺旋,底部用小字写出大萧条数字",
        "节俭悖论的核心张力:对个人最优的储蓄率,放大到全民就是灾难。1929 年美国储蓄率翻倍加货币流通减速,4 年 GDP 跌三分之一。判断口诀:你的支出是别人的收入——都省,就都赚不着。")


def _fig_o14_minsky():
    """ch14:明斯基三档融资:稳定时期的慢性下沉。"""
    body = """            <text x="450" y="34" text-anchor="middle" fill="#eef4f8" font-size="19" font-weight="800">明斯基三档:长期稳定本身就是风险培养基</text>
            <!-- 三档台阶从左优到右崩 -->
            <rect x="70" y="100" width="220" height="180" rx="10" fill="rgba(143,179,122,0.2)" stroke="#8fb37a"/>
            <text x="180" y="130" text-anchor="middle" fill="#b5d3a0" font-size="15" font-weight="800">① 对冲性融资</text>
            <text x="180" y="158" text-anchor="middle" fill="#bcc9dd" font-size="12">收入足以付本息</text>
            <text x="180" y="180" text-anchor="middle" fill="#bcc9dd" font-size="12">例:国债、优质房贷</text>
            <text x="180" y="210" text-anchor="middle" fill="#8fb37a" font-size="13" font-weight="800">稳当</text>
            <text x="180" y="240" text-anchor="middle" fill="#8499bd" font-size="11">太平初期的样子</text>
            <rect x="340" y="100" width="220" height="180" rx="10" fill="rgba(240,201,106,0.2)" stroke="#f0c96a"/>
            <text x="450" y="130" text-anchor="middle" fill="#f0c96a" font-size="15" font-weight="800">② 投机性融资</text>
            <text x="450" y="158" text-anchor="middle" fill="#bcc9dd" font-size="12">收入只够付利息</text>
            <text x="450" y="180" text-anchor="middle" fill="#bcc9dd" font-size="12">本金靠借新还旧</text>
            <text x="450" y="210" text-anchor="middle" fill="#f0c96a" font-size="13" font-weight="800">靠续贷维持</text>
            <text x="450" y="240" text-anchor="middle" fill="#8499bd" font-size="11">繁荣中期的样子</text>
            <rect x="610" y="100" width="220" height="180" rx="10" fill="rgba(238,136,119,0.15)" stroke="#ee8877"/>
            <text x="720" y="130" text-anchor="middle" fill="#ffb4b4" font-size="15" font-weight="800">③ 庞氏融资</text>
            <text x="720" y="158" text-anchor="middle" fill="#bcc9dd" font-size="12">投入连利息都不够</text>
            <text x="720" y="180" text-anchor="middle" fill="#bcc9dd" font-size="12">全靠资产升值卖出</text>
            <text x="720" y="210" text-anchor="middle" fill="#ee8877" font-size="13" font-weight="800">一停下来就崩</text>
            <text x="720" y="240" text-anchor="middle" fill="#8499bd" font-size="11">泡沫末期的样子</text>
            <!-- 下滑箭头 -->
            <path d="M 200 90 Q 450 60 700 90" fill="none" stroke="#ee8877" stroke-width="3" stroke-dasharray="8 6" marker-end="url(#arrowO2)"/>
            <text x="450" y="80" text-anchor="middle" fill="#ee8877" font-size="12.5">太平越久,社会整体向下沉</text>
            <!-- 底部历史例证 -->
            <text x="450" y="318" text-anchor="middle" fill="#bcc9dd" font-size="12.5">2008 美国次贷、2016 前中国影子银行、1989 日本地产:三场危机都是从“看起来稳”的那一级慢慢滑下来的</text>
            <defs><marker id="arrowO2" markerWidth="9" markerHeight="9" refX="8" refY="4.5" orient="auto"><path d="M0,0 L9,4.5 L0,9 Z" fill="#ee8877"/></marker></defs>
            <text x="450" y="352" text-anchor="middle" fill="#8499bd" font-size="12.5">“明斯基时刻”不是危机发生的那一下,而是三档结构垮塌前的最后静默</text>"""
    return concept_figure(body, "0 0 900 380",
        "明斯基三档融资台阶图:对冲性融资收入付本息最稳,投机性融资收入只付利息,庞氏融资收入连利息都不够,顶部红色箭头表示太平期心灵会向最崩处下滑",
        "明斯基的三档融资按稳健度排队:对冲、投机、庞氏。麻烦在于长期稳定会让整个金融系统自发地从左往右滑——不是因为谁都变贪婪了,而是每一次风险证明都被平安否认。能用历史数据看到的最后一格,就是明斯基时刻。")


PATCH_O = {
    14: [("通胀还是通缩", _fig_o14_thrift),
         ("稳定本身会生出不稳", _fig_o14_minsky)],
}
...
PATCH_L = {
    ...
    26: [("训练、预测和回测不要混", _fig_l23_pipeline)],
}
...
_merge_figures(CONCEPT_FIGURES, PATCH_O)
