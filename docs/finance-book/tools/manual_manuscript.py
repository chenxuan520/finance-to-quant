    ...
    ch(
            ...
            [
                ...
                (
                    ...
                    )                    + callout("超额收益要分两本账", "名字同为超额,指增的超额来自“跟稳指数之外再多选几只对的股”,中性的超额来自“多空抵消后剩下的选股手艺”——口径不同、成本不同、风险也不同。看产品材料时先问一句:这份超额走的是哪个口径、用什么工具出来的、对冲费用占了几成;再问这个水平能不能持续,而不是盯着宣传页上“超额 5%”那一个数。", "tip")
                ...
                (
                    ...
                    )                    + callout("高收益与低回撤很难同时存在", "年化 20%、最大回撤 4%,这组数字最常见的出没地是截图和营销页,因为实盘里它几乎不存在。两者都漂亮时,多半是三种原因之一:统计窗口刚好赶上一段平滑行情、样本太短还没遇到坏天气、或者回测里混进了当时根本不可能拿到的信息。把“收益 ÷ 最大回撤”这个比值当作判别器:长期能做到 3 以上已是头部水平,宣传页动辄 5 甚至 10,就当它是需要举证的例外,而不是默认值。", "warn")
,
                ),
            ],
            [
    ...
    ch(
            ...
            [
                ...
                (
                    ...
                    )                    + callout("远期买的不是方向,是确定", "签了远期,三个月后价格涨跌都与你无关——你买的从来不是“赌对行情”,而是“不用再猜行情”。谁签远期?怕波动的生产者、要锁成本的企业——他们付的价格是用未来可能到手的便宜,换今天就能把账算平。", "note")
,
                ),
                (
                    "远期和期货有什么区别",
                ...
                (
                    ...
                    )                    + callout("LTCM 的教训不在智商,在杠杆", "两个诺奖得主 + 一位前美联储副主席,模型精度业内顶尖——他们栽的不是算错,是仓位重到不允许算错。市场朝模型外走出 10%,几十倍杠杆就把净资产打穿;同样的冲击落在无杠杆仓位上,只是一个月的坏行情。杠杆真正改写的不是期望收益,而是“有没有资格等到自己对了”的时间垫。", "warn")

                    + callout("保证金不是少缴费,是抵押金", "保证金不是你为合约付的“价格”,而是你押在那里、证明自己扛得住波动的抵押金:本金 1 万撬动名义 10 万,等于把波动放大十倍。方向错 10%,押金全部亏光,以外还要倒贴。每天都结算一次,你今天亏的今天就从你的账里划走——这个市场里没有“等一等再看”,只有“补不补得上”。", "warn"),
                ),
                (
                    "套期保值、投机和套利",
                    p(
                    ...
                    )                    + callout("套保对齐的是风险,不创造收益", "渔民 10 元卖出期货、现货跌到 8 元就平仓回补——期货那边每条赚 2 元,现货这边每条少赚 2 元,一赚一亏互相抵消,净收入最终稳定在 10 元。这不是变出来的收益,而是把“未来的不确定性”按今天的价格封存了一笔;套保的收益不在弯上有多少拐弯,而在直线稳不稳。", "note")
                ...
                (
                    ...
                    )                    + callout("换月的那一跳不是涨跌,是价差", "近月合约到期前,价格压力会逐渐转移到远月合约——图表上那道翘起不是行情、是两份合约之间本来就该有的价差。回测里如果没把这种移仓换月处理好,资金曲线会凭空出现一段大涨/大跌——那不是市场给你的礼物,是数据拼接的裂缝。做期货回测时必须先问:计算收益时,两处价格真的是同一件东西的吗?没处理好,报告里的那条曲线多半不是策略的功劳。", "warn")
...
,
    ...
    ch(
        ...
        [
            (
                "先分清三个账户概念",
                callout("读到这部分之前该知道什么", "这一章把交易的每一道手续——账户、撮合、清算、交收、费用——逐项拆开,所以它对程序读者的重点是理解金融系统对“一次买卖”到底要求你记住什么状态。读懂它,你再写任何交易系统或回测框架,都会更懂为什么书上的代码要长这样。", "tip")
                + p(
    ...
    ch(
        ...
        [
            ...
            (
                ...
                )                + callout("新手期慎用真做空", "书里说做空是为了帮你把账看全,不是让你直接上手——零章再试,只混股指期货;模拟盘先,实盘后;本金上限设小。真正对零基础读者来说,这几年足够用来把你对做多本事的估值模型打磨到位,再看是否需要在更大轮子里加空。", "warn"),
            ),
            (
                "最小交易单位和价格单位",
                p(
    ...
    ch(
        ...
        [
            ...
            (
                ...
                + callout("研究指数总涨跌时,别看点数,看收益率", "真正要问的是:这段时间点画出来的涨幅,拆开成百分比是多少?比如指数从 2000 点到 3000 点,真正有意义的信息是 50% 而不是 3000 这个绝对值。点数的最大用处,是让你可以快速比较同一天下这一点相对起始发生了什么,以及跨时期大致判断“现在是牛市的第几段”——除此之外,别被点数的“大”“小”牵着情绪走。", "tip")
            ),
            (
                "市值加权: 大公司说话更大声",
                p(
    ...
    ch(
        ...
        [
            (
                "因子不是魔法,是一列有口径的数据",
                callout("读到这部分之前该知道什么", "第五部分要把“判断有没有效”这件事引入数据、回测和真实交易这个完整循环。读这章时,关键是把每一条建议还原到实际规则——数据怎么对齐、成本怎么算、股票池填哪只名字、它最后多信、样本外能不能撑住。看懂这一部分,你才算真正进入了本书的世界。", "tip")
                + p(
...
ranked = sorted(stocks, key=key, reverse=True)           <b class="cw-tag">3</b></code></pre>
    ...
    ch(
        ...
        [
            (
                "增长的三个阶段: 人力、资本、生产率",
                callout("读到这部分之前该知道什么", "宏观本身不是让你变成政策解读员,而是把国家增长、通胀、利率、美元、黄金这些变量拆成清晰的发射机——读懂它们之后你会看到资产价格是怎么被折现、相关性怎么被算盘调动、甚至连“明天股票为什么跌”都能拆到变量。第四部分的任务,就是让你逐渐会用宏观思维做投资、写多资产回测。", "tip")
                + p(
...
ch(
            ...
            [
                ...
                (
                    ...
                    + callout("先看这三行能不能盖章", "你的保险够不够,只要先回答三个问题:① 出了十万医疗费你能不能付;② 家人失去你的工资后几年内能不能活得下来;③ 事故后你或家人有没有一笔过渡费。三行都答得上,你再谈增额寿、教育金、理财;答不上的,先把基础补齐再谈下一格。", "tip"),
                ),
                (
                    "读保单:先把五件事找出来",
                    p(
...
ch(
            ...
            [
                ...
                (
                    ...
                    + callout("打新的题从不复杂:你现在拿一万市值,能中就是几千块,中不了钱就是白拿一天。但问题从不在能不能中,而在你想清楚想卖的地方——持一只股票拿一周、还拿一年,二者分开,盈在盈乎,也就截然相反。", "tip")
                ),
                (
                    "停牌与复牌:屏幕上突然消失的按钮",
                    p(
...
NARRATIVE_CASES = {
    ...
    38: (
        "Case: 让全世界最保守的基金差点倒闭的,不是股票,是利率上调",
        p(
...
CONCEPT_SCENES = {
    ...
    34: [
        (
            "把“谁赢大选”做成一张可交易合约——预测市场的信息聚合,比民调快",
            p(
    ...
    6: [
        ...
        (
            ...
            + callout("产品力输给资本力的三种常态", "烧钱换市场、补贴大战、资本碾压,是近十年消费和平台经济里反复上演的剧本:滴滴 vs 快的、拼多多 vs 当当、蜜雪冰城 vs 一群高端茶饮、亚马逊早年常年亏钱压着传统零售打。这不说明好奶茶不重要——它是最终活下去的那家公司的本钱;但抵达终点之前,先决条件是你得活到终点。", "warn")
            + p(
...
GLOSSARY = [
    ...
    ("头寸", "你当前持有的仓位本身——账上那笔已经开在桌上的牌。头寸不分多空,持有的多头、空头都叫头寸。"),
    ("做多", "先买后卖,赌涨;价格涨了才有赚头。这是最熟悉的交易方向。"),
    ("做空", "先借来卖掉、跌了再买回来归还,赚“从高到低”的差价;股票端叫融券,期货端直接开空。"),
    ("开仓", "从无到有地建立一笔头寸——第一次买入或第一次卖出合约的那个动作。"),
    ("平仓", "把已经开在账上的头寸原路了结:多头平仓=卖掉,空头平仓=买回来还。爆仓、强平说到底都是别人替你做了这笔平仓。"),
    ("保证金", "开仓前押在券商/交易所那里的一笔抵押金,用来证明你扛得住亏损。亏到保证金线以下,就要么补钱要么被强平。"),
    ("强制平仓", "当你的保证金不够也没及时补钱,券商/交易所不等你同意,直接把头寸强平掉——这就是“爆仓”的那一下。"),
