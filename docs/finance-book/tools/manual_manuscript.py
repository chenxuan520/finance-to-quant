...
,
    ...
    ch(
        ...
        [
            (
                "因子不是魔法,是一列有口径的数据",
                callout("读到这部分之前该知道什么", "第五部分要把“判断有没有效”这件事引入数据、回测和真实交易这个完整循环。读这章时,关键是把每一条建议还原到实际规则——数据怎么对齐、成本怎么算、股票池包含哪些股票、结果有多可信、样本外能不能撑住。看懂这一部分,你才算真正进入了本书的世界。", "tip")
                + p(
...
def test_no_trade_when_limit_up():
    ...
    for code in selected:
        target[code] = min(target_weight, 0.03)</code></pre>
            ...
            (
                ...
                + """
                <p>前面说的“T 日收盘出信号、T+1 开盘成交”,要同时落实信号时间和成交价格。下面是 <code>examples/ex02_index_enhance.py</code> 的时序摘录,省略号处的逐笔成交逻辑见完整脚本:</p>
                <figure class="code-walk reveal">
    ...
    if pending is not None:
        ...
        open_nav = cash + sum(values.values())      <b class="cw-tag">1</b>
    ...
    strat_nav.append(cash + sum(
        ...
        for c, qty in holdings.items()))            <b class="cw-tag">3</b>
    ...
    if t > WARMUP and t % REBALANCE_EVERY == 0 and t &lt; n_days:
        ...
        pending = {s.code: 1.0 / len(selected)
                   for s in selected}              <b class="cw-tag">4</b></code></pre>
                  <ol class="code-walk__notes">
                    <li><strong>旧持仓先按开盘价估值</strong>:隔夜涨跌属于昨晚已经持有的股票。<code>pending</code> 是前一日收盘后的信号,不能让它倒回昨晚成交。</li>
                    <li><strong>成交才改变股数和现金</strong>:按当日开盘价先卖后买,买卖每边按成交额扣费;现金不够支付买入金额和费用时,同比缩小买单。发出信号时不扣费。</li>
                    <li><strong>收盘净值来自实际账本</strong>:现金加股数乘收盘价。昨收 100、今开 110、今收仍为 110,今天新买入的股票价格收益为 0,不能把隔夜的 10% 算给它;两次调仓之间,股数保持不变。</li>
                    <li><strong>收盘后只生成待执行目标</strong>:<code>factors_at</code> 只读取 <code>[0, t]</code> 内的数据,目标等到下一交易日开盘执行。样本末尾没有下一交易日,就不再生成待成交目标或扣费。</li>
                  </ol>
                </figure>
                """
                + p(
