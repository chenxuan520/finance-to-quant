...
def callout(title, body, style="note"):
    return f'<div class="callout callout--{style} reveal"><span class="callout__title">{title}</span><p>{body}</p></div>'
    ...
    ch(
            ...
            [
                (
                    ...
                    )                    + callout("先想清楚再发钱", "发钱这个念头真正要看的不是屏幕上的数字变大,是仓库里的实物有没有变多。盯住“真实商品有没有多生产一件”这件事,就不会被任何“发钱刺激”的故事晃花眼。这条直觉,在后面章节的通胀、量化宽松的讨论里,每次都会再回来。", "note")
                    ...
                    + callout("看完后应该记住什么", "一个社会真正变富,不在于票据变多,而在能生产、保存、交换、使用的真实东西变多。\n带着这句话往后读:每一章再讲一个概念时,先问它最终落在哪一件真实东西上——是生产率,是风险转移,还是只是数字在屏幕上换位。", "tip"),
                ),
                (
                    "储蓄不是抠门,是给未来留出空间",
                    p(
                    ...
                    )                    + callout("拿一天换一张网", "你现在手里少一条鱼,明天网成之后每天多抓两条。短期的难受,换来的是长期能自由支配的产出曲线——这就是储蓄到投资到回报的最小闭环。下次你看任何“项目”,都把这条线再摆一遍:今天的代价是什、明天的增量是多少、能持续多久。三个格子里空格,就不该投。", "tip")
                ...
                (
                    ...
                    )                    + callout("金融只是在放大已经在发生的事", "金融不造鱼,金融放大鱼。给编网的人资源,网就多织几副;给吹牛的人资源,吹牛就被放大成全岛的亏损。所以金融的好坏不取决于金融本身,取决于它把钱递给了谁的计划。", "note")
                ...
                (
                    ...
                    )                    + callout("模型分数第一关:它是从哪来的", "任何策略里那个精致的“上涨概率”,先别问高不高,先问它从哪里来——是盈利变化、风险补偿变化、流动性变化,还是纯粹的历史噪声。如果说不出来源,就把它当成噪声对待。这是后面所有章节里,判断任何模型输出时的同一个起点。", "tip")
    ...
    ch(
            ...
            [
                ...
                (
                    ...
                    )                    + callout("通胀不是均匀洒下来的水", "通胀对人群从来不是平摊的。攥着现金和固定工资的人先受伤;欠着固定债务的人被默默免债;最先拿到新钱的人(往往是发债方和资产持有人)拿走最大一块。所以“放水”两个字背后,是一次有方向的财富再分配,不是所有人一起缩水。", "warn")

                    + callout("韩元日元为什么面额这么“大”", "1 韩元大约兑 0.005 元人民币,1 日元大约兑 0.05 元——面额大不是因为日韩经济弱,是战后通胀反复磨损货币单位的结果。反直觉的点是:货币面额反映的是历史通胀轨迹,不是今天的经济强弱。所以看一国货币先别被面额吓到,豪华大数字的货币,往往只是历史包袱大。", "note")
                    + callout("坏钱驱逐好钱的现代版本", "法定把实际价值不同的两种货币按同一面值流通,人们自然先花便宜的、留下贵的,格雷欣法则就来了。今天它也活着:稳定币里流通最广的往往是最不透明的那一家,刚兑不清的“高收益理财”把本该去正规渠道的钱吸走。定价机制容不容许价差存在,这是判断某一类资产是不是“劣币”最直接的标准。", "note"),
                ),
                (
                    "价格是信号,不是敌人",
                    p(
                    ...
                    )                    + callout("按住价格潜水艇也还在动", "把价格固定,以为事情就这样控制住,那才是真正危险的地方。价格被按住时,你不知道哪里缺得最厉害——供给方永远不告诉你那头货源少了多少,要等到货架全空,你才反应过来。所以看所有价格管制新闻,先问“缺货被藏在哪里”,而不是“价格稳住了没有”。", "warn")
                ...
                (
                ...
                )                    + callout("先记住这句话", "一个社会真正变富,不是因为票据变多,而是因为能生产、保存、交换和使用的真实东西变多。", "note")

                    + callout("货币面额是历史包袱,不是国力徽章", "1 韩元约兑 0.005 元人民币,1 日元约兑 0.05 元——日韩货币面额很大,不是它们经济差,而是它们在历史上经历过高通胀,单位被一点点磨小。判断一种货币的强弱,永远换算到同一份购买力再比较,别被“一万韩元”的数字虚张声势吓到。", "note")
                    ...
                    + callout("加薪 5% 应对通胀 8%,实际购买力是亏的", "精确做法不是 5 - 8 = -3,而是用除法——(1 + 5%) ÷ (1 + 8%) ≈ 0.972,购买力只剩约 97.2%。口算用减法,买入大件、签约工资、写研究代码里,一律都用除法;这是最容易、也最容易被忽视的日常细节。", "tip"),
                ),
                (
                    "为什么钱多了不一定更富",
                    p(
                    ...
                    + callout("人人都只储蓄,社会反而变穷", "个人勒紧裤腰带是美德;所有人同时把支出全收回来,就是经济的集体刹车——你的支出减少,正是别人的收入减少。这叫节俭悖论,讲的是个体理性和集体理性的冲突,也是大萧条通缩螺旋可以恶性循环的原因之一。", "warn"),
                ),
                (
                    "通胀数据如何进入量化",
                    p(
                    ...
                    )                    + callout("任何跨年份的回测,先除掉通胀", "名义价涨 10%,物价指数同期涨 5%,这笔钱其实只多了 5% 的购买力——名义收益和实际收益必须分开报。你拿到任何历年回测图表,先问它最后一列是不是 CPI 调整后的实际净值,不是的,上一层的结论都得打折。", "tip")
    ...
    ch(
            ...
            [
                ...
                (
                    ...
                    )                    + callout("利率其实是违约概率在借钱上的价格", "放 10 笔贷,其中 1 笔回不来,如果前 9 笔的利息要覆盖那 1 笔的损失,利率就必须比“无风险利率”高出一截。利差、信用评级、抵押品要求的本质,都是在对“能不能按时还回来”这次违约概率做一次集体报价。", "note")

                    + callout("回到利息的两个本源", "钱的时间价格(你今天拿不到,我补你)跟违约风险补偿(你可能拿不回来)相加,才是你实际付出的利率。以后看到任何一个利率数字,先拆开问:这个时间成本是多少、这个风险补偿是多少——你立刻就能判断它高得还是便宜得有理。", "tip"),
                ),
                (
                    "好债和坏债的区别",
                    p(
                    ...
                    )                    + callout("先问回报,再问利率", "项目预期回报率 10%、利率 5%,差为正,债是杠杆;回报率 3%、利率 5%,差为负,债是绞索。两个判断都要成立,最关键的是“回报率”这一行的数据必须自己核过,才不算自欺欺人的高收益诱惑。", "warn")
                ...
                (
                    ...
                    )                    + callout("老债为什么会跌,跟信用无关", "现在市面上能给 10% 的新机会来了,手里那张确定一年后还 105 元的老债,今天只值 105 ÷ 1.10 ≈ 95.45 元。你什么都没做错,但价格就是跌了——因为折现的分母变了。这是债券价格对利率反向的核心机制。", "tip")
                ...
                (
                    ...
                    )                    + callout("利差不是收入,是保险单", "国债 3%、同类公司债 6%,那三个点的差距是市场替你承担违约风险的报酬。利差走扩,不是哪一家公司出了事,而是市场对整个链条的红灯亮起——读固收指标,先盯的就是利差走扩/收窄的方向。", "warn")
    ...
    ch(
            ...
            [
                (
                    ...
                    + callout("资产负债表看懂银行,再看一切金融机构", "资产端回答“钱去了哪”,负债端加资本端回答“钱从哪来”——这套镜像问你某家银行、某家基金、某家租赁公司都一样生效。下次看金融行业报道,先找它的这张表,后面的所有结论才有根。", "tip"),
                ),
                (
                    "一张极简资产负债表",
                    p(
                    ...
                    )                    + callout("先烧股东的钱,再烧储户的钱", "坏账 5 元,资本 10 元罩住;坏账 12 元,资本被打穿,储户才开始赔。银行的“稳”不看网上口碑、不看招牌,看它垫在最底下的那一层资本厚度——查核心资本充足率,数字监管每季度都公布,比口碑可靠得多。", "warn")
                ...
                (
                    ...
                    )                    + callout("先放贷，再有存款", "银行放 100 万贷款的同一瞬间，小店账户里就多了一笔 100 万存款——不是把别人的存款搬过去，是账本上同时新记了两笔。这就是为什么宏观上“货币供应量大增”通常发生在信贷扩张之后：先有贷款这个动作，后有存款这个结果。把因果顺序记反，后面关于货币乘数的所有推导都会跟着错。", "note")
                ...
                (
                    ...
                    )                    + callout("没有人做错，系统照样崩", "你不用判断别人会不会跑——你只要确认一旦跑晚了你会亏，就已经够了。挤兑的骇人之处在于，它不需要任何人愚蠢或贪婪，只需要每个人都正常地对自己负责。这也是存款保险的设计初衷：不是让银行更安全，而是让“先跑”这个动作不再有利可图。", "warn")
                ...
                (
                    ...
                    )                    + callout("四道闸各管一段，缺一个都会漏", "存款保险兜住“储户别跑”，最后贷款人兜住“银行一时没钱”，资本监管兜住“股东先垫”，流动性覆盖率兜住“扛过 30 天大撤退”。四个名字背后是四道不同的防线，防的是同一条踩踏链条上的四个环节。把它们混为一谈，就说不清 2008 年和硅谷银行到底死了在哪一环。", "note"),
                ...
                (
                    ...
                    )                    + callout("水位不是生产率", "央行能放宽融资条件、压低利率、让资产价格被水托起来——但破船不会因此变快。货币政策改变的是金融条件，变不出技术进步和真实需求。听到“放水”新闻时先分清：它接下来推升的是哪里的价格，以及它管不了的那些事最终会不会决定这次水位的持续度。", "warn")
,
                ),
                (
                    "为什么量化也要懂银行和央行",
    ...
    ch(
            ...
            [
                ...
                (
                    ...
                    )                    + callout("宣传语是情绪,六问是事实", "拆开任何一只产品的宣传页，收益率、回撤、管理人名校背景全是别人想让你看见的。真正需要你回答的六句：钱最后流向什么资产？谁在动它？放在哪家托管？费用从哪边扣？最坏情况是什么样？出问题我能找谁？答完这六句，宣传页上的每一个亮点你都能判断该不该信。", "tip")
,
                ),
                (
                    "风险不会消失,只会转移或隐藏",
                    ...
                    )                    + callout("对冲的账永远要两本一起算", "期货账户亏了 1 亿，别急着喊“对冲失败”——那往往意味着现货这头你因为油价上涨而多掏了 1 亿，一亏一赚合起来是平，这就是对冲的目的。把两边拆开看的评价就像只称一边称，先合起来算，再讨论是不是失败。", "warn")
                ...
                (
                    ...
                    )                    + callout("中间价的残酷在于它谁也不准确", "不知道好坏，市场只能报一个对两类人都不公平的利率——对诚实的偏高，对会赖的偏低，于是好客户先被这个价格吓跑。这不是银行的失误，是信息不对称本身的成本；所以想见到合理的利率，得先把“谁更可能还钱”制度化地讲清楚，审计和评级干的就是这件事。", "note")
                ...
                (
                    ...
                    )                    + callout("把最坏情景写下来，再加杠杆", "资产跌 20%，5 倍杠杆下你清零出局——这不是极端案例，是每天都会重播的算术。杠杆不改变方向判断，只放大结果。所以在真正加杠杆之前，先把这条线写下来：跌多少我会出局？这个数写不出来，你就还不配碰它。", "warn")
    ...
    ch(
            ...
            [
                ...
                (
                    ...
                    )                    + callout("边际成交给所有人定价", "今天真正易手的只有 500 万股，但这一笔成交价被全部 10 亿股拿去参考，你账面浮盈浮动的原因不是有人真的卖空了你的股票，是因为“最近一次成交价”被所有人当成价格的锚。这就是 K 线显示的市值是全体重估的结果，不是当日真实出货量的总和。", "note")
                    ...
                    + callout("钱进谁口袋，就是哪个市场", "IPO 时你的钱进了公司账户，为公司发展添了燃料，这是一级市场 你在 App 里买一股，钱进了卖方投资者的账户，这是二级市场 同样是“买股票”，前者是融资，后者是换手——这两件事的经济含义完全不一样，把他们混为一谈你就看不懂企业当年为什么市值更高还增发。", "tip"),
                ),
                (
                    "成交不是终点,后面还有清算和交收",
                    p(
                    ...
                    )                    + callout("轧差是金融里最朴素也最重要的一个动作", "甲卖乙 1 万股、乙卖丙 1 万股、丙卖甲 5000 股——囤着逐笔过户要 3 次，清算只搬 1 次，因为大家的净头寸被轧平。默认净额结算不是技巧，是基础设施——没有中央对手方清算的市场，换手率的所有假设都不然。做交易策略，你也要问自己：你的对手风险假设是不是隐含的已经完成对了？", "tip")
                ...
                (
                    ...
                    )                    + callout("下单那一刻，你的钱已经不是你的了", "委托买入 1 万股 × 10 元，券商先冻结 10 万——成交 6000 股后，6 万变成股票，4 万还在冻结状态，既不能动用也不能买别的东西。这就是为什么“可用资金”和“总资产”要靠两个不一样的数字列出来——冻结状态中的钱不是你的可用资源。", "note")
                ...
                (
                    ...
                    )                    + callout("亏钱的往往不是模型，是“管道”", "方向对、模型对，最后却亏钱——很多情况下模型没问题，坏在订单没发对、资金对不上、数据没同步、托管接口出了状况。研究策略时把这个问题写进你的检查清单：上实盘前一天，先核对所有交易执行、资金状态、风控报错是不是真的在预期的轨道上跑，而不是凭感觉觉得没问题。", "warn")
    ...
    ch(
            ...
            [
                ...
                (
                    ...
                    )                    + callout("股价绝对值什么也说明不了", "十元股和百元股的区别只是总股数不同。便宜与否看的是市值(股价 × 股本数)和利润的关系,不是单张价格。同等市值的两家公司,股本切成 5 元一份还是 200 元一份,只是票面习惯 ——跟公司贵贱无关。", "warn")
                ...
                (
                    ...
                    )                    + callout("利润涨、股价跌,是估值倍数在动", "利润增 20%、市盈率从 20 砍到 15——股价 = 6 × 15 = 90,比原来的 100 跌 10%。公司盈利和市场愿意给的倍数是两个独立的变量。盈利按季度变化,倍数按市场情绪变化,两条线叠加才决定股价。你只盯盈利增长,就有可能被倍数收缩吓一跳。", "note")
                ...
                (
                    ...
                    + callout("估值是价格,不是价值本身", "投后估值 10 亿不等于公司账上有 10 亿——它只是这轮参与方达成的入场价;真正锁进合同的,是优先股、清算优先权、董事会席位这些条款。看到巨额估值不要先欢呼,先看条款里谁拿到了更厚的保护垫,那才是熊市里负责兑现的部分。", "warn"),
                ...
                (
                    ...
                    )                    + callout("因子有效性要问三个问题", "一个因子历史上排得好,离不开三类解释:补偿了某种风险(承担了某种损失)、利用了某种行为偏差(市场参与者反复犯的错)、捕捉了某种信息结构(尚未普及的规律)。说不清楚哪一种,就先当它只是这一段数据的巧合。多数看起来有道理的策略后面,都藏着被过拟合的历史统计噪声。", "tip")
    ...
    ch(
            ...
            [
                (
                    ...
                    )                    + callout("先比例,再看总额", "公司想再募 10 亿——股价 50 元时新发 2000 万股,股价 25 元时新发 4000 万股。关键不是金额,是总股本的比例:比例小,老股东只是被轻轻动了一杯羹;比例大,你的权益被深度摊薄,要先弄清楚给你的新股背后,到底换来多少钱。", "tip")
                ...
                (
                    ...
                    )                    + callout("复权让历史价格可以比", "分红、送股、拆股之后,100 元时代的股价和 50 元时代的股价没法直接放在一起比——名义股价突然暴跌了一次,但实际上你并没有亏。复权的目的只有一个:把这种因为一个除权/拆股等技术动作引发的价格断层修掉,让历史价格真正可比。做长周期回测必须复权,不复权,你看到的很多大跌很可能只是除权造成的幻觉。", "warn")
                ...
                (
                    ...
                    )                    + callout("折现率变 1 个点,估值差不是 1 个点", "公司未来每股稳定分红 10 元——要求回报 5% 的时候,这串现金流值 200 元;改成 10%,直接砍半到 100 元。估值对折现率不是“差不多的关系”,是剧烈的跷跷板:它动了 5 个百分点,价格可以差一倍。同样一句话,放到央行利率决议和长期债券市场,完全不同的含义。", "warn")
                ...
                (
                    ...
                    )                    + callout("牛顿的区别,不在第一次,在第二次", "南海泡沫早期入场,获利丰厚,冷静离场——这说明他不是不懂风险;后来在急涨中控制不住情绪入场,亏光收场——这说明,在投资上,聪明不代表你能抵抗蹝人性的弱点。把牛顿的教训简化成一句话: 理性和自制力,是两件不同的事,你都需要它们。", "warn")
    ...
    ch(
            ...
            [
                ...
                (
                    ...
                    )                    + callout("债券定价的根本逻辑是跷跷板", "一张债,票息固定,today你买入的价格越低,你能拿到的收益率就越高;价格越高,收益率越低。这条反向关系不需要任何高深的数学——它是你下半本书里,久期、利率、甚至央行加息降息都要反复用到的基础观念。", "tip")
                ...
                (
                    ...
                    )                    + callout("“不违约”不等于“不会跌”", "长期国债信用没问题,但价格会跟着利率变——利率升 1%,久期 7 的长债跌约 7%。这不是它信用变差了,是它输给了更高的新利率。买债券要分开看:信用风险问“它能不能还”,利率风险问“利率会不会变动”,重点是不一样的。", "warn")
                ...
                (
                    ...
                    )                    + callout("利差是市场把不放心写在价格上", "国债 3%,公司债 6%——多的这 3 个点不是补偿,是市场为你承担的违约风险收的保险费。利差走扩的时刻,等于市场在说:整个系统的信用热情都在退,不是某某公司的评级下调,需要走的, 不是 国债,是所有的风险资产都要重新定。固收里最先盯住的,就是这个价。", "note")
                ...
                (
                    ...
                    )                    + callout("美债是全世界的价 锚", "从A 股到港股,从黄金到新兴市场的债卷市场——所有资产定价 的折现率,都跟 10 年期美债对标。 不是因为美元有多优越,是因为它做到规模最大,流动性最深,接受度最广。看美债收益率, 你看的不是美国,是全球所有资产的一起折现的世界。", "note")
    ...
    ch(
            ...
            [
                ...
                (
                    ...
                    )                    + callout("净值高不等于贵,只是积累不同", "净值 3 元的基金,一万买到 3333 份;净值 1 元的基金,一万买到 1 万份。你投入的资金能买到的权益份额是相同比例的——净值高低只反映这只基金活了多少年、复利滚了多久,跟贵不贵没关系。比基金永远用“同期涨跌幅”这个口径,不拿净值绝对值来比。", "note")
                ...
                (
                    ...
                    )                    + callout("指数基金的价值是承认“跟不到也可以”", "它不想打败市场、也不想跑赢市场——它只想追上市场。这在逻辑上是绝多数主动基金无法承诺的事情。 在股市长牛里,它赚的是跟上整个市场的钱; 在股市波动里,它赚的是不被恐惧和贪婪影响情绪的钱。而对大多数零基础的普通人而言,指数化定投比追各式各样的策略基金都更稳定。", "tip")
                ...
                (
                    ...
                    )                    + callout("超额收益的“两本账”是会上瘾的", "同为超额,指增是靠选股能力,中性则靠多空对冲都要两匹谁来。名字一样, 路径不同,成本不同,风险也大不相同。买入强奸报告时,先问一句: 收益的绝大部分是来自哪里?他走的哪个口径?再问它是不是可持续, 而不是看“超额5%”这一点有多少.", "tip")
                ...
                (
                    ...
                    )                    + callout("高收益+低回撤几乎是矛盾", "年化 20%、最大回撤 4%,最容易也最多地出现在截图和营销页上: 实盘里这种组合几乎不存在。如果两个数字都好,要么是行情周期的问题让曲线平滑了,要么是那个回测里用到了明天的数据. 长期稳定的高收益不免费,宣传出来的““圣杯””背后几乎都有一套你没看见的坑 —— 每年预期均 20% 还很稳定的资产,多短款可能是神仙.把这两条当作判别器,并不是噪音或错:能在实际中做得很久的策略,其回撤之和非常必要,就有所谓该被花利润给出。", "warn")
,
                ),
            ],
            [
    ...
    ch(
            ...
            [
                (
                    ...
                    )                    + callout("远期从来不是对市场的预测", "你签一份远期合约,交割时价格跌了 —— 你没有“赚”,你没有“亏”,你只是把未来的价格锁定—— 远期合约的意义不是预测市场,是让未来的不确定性变成确定性。 签远期的人都是目标成本确定性优先,而不是“我看多/看空”. 所以签三个月价格,也仅仅就是为了“我不用在三个月内再次决策”的便利。但你不确定的是,锁定价格这件事本身,对整个行情没有指导意义。", "note")
,
                ),
                (
                    "远期和期货有什么区别",
                ...
                (
                    ...
                    )                    + callout("LTCM,就是高杠杆 + 低概率侥幸", "两位诺贝尔经济学奖的创始人,加上杠杆不到 10% 的市场走势,想象他研还是爆了 —— 仓,头寸集中,的本身就瞬间,杠杆纵向的是它隐藏性,并不是不可能的大败结局,关键不在也信自己诺贝尔经济学,不是聪明跟将那基本的硬事情放手看坏实测绝对没有给钱能翻盘", "warn")

                    + callout("保证金低不等于安全", "1 万保证金炒 10 万合约——资金少,张跌 10% 就亏光。保证金不是“少缴费”,而是你为了控制大额合约而必须支付的第一笔押金。 本金 1 万 → 名义 10 万,方向错 10% 就意味着保证金完全消失,额外的 “亏空” 此时就全部变为自费. 以后再想用“只有 1 万”来安慰自己,先想清楚: 名义那 10 万的波动该由谁承担?", "warn"),
                ),
                (
                    "套期保值、投机和套利",
                    p(
                    ...
                    )                    + callout("套保的意义是锁住风险,不是收益", "10 元卖期货、8 元回补——期货每条赚 2 元;同时现货这批鱼从你手 8 元的价格卖出,你本可以从 10 到只能 8——一赚一亏,合起来平了. 这不是特殊的收割方式, 这是交易成本. 你能把这条搞懂—— 能彻底搞清楚 它不是市场变出的收益, 是套保给的代价.", "note")
                ...
                (
                    ...
                    )                    + callout("换月是远月价格高于近月的价格", "近月合约快到期时的价格势能,通常会逐渐转移到远月合约——它不是涨, 是价差. 回测里没对这种市价变化处理的资金曲线就可能是假的. 你做期货回测,一定要问的是: 是按哪个价来计算的了换月收益、是远月的涨幅还是近月本身的涨跌,是一张残格历 史资 产到底要不要平?", "warn")
    ...
    ch(
            ...
            [
                (
                    ...
                    )                    + callout("权利金是“给不确定性付房租”", "2 万元你买的不是 100 万房子,是三个月内能按 100 万入场的权利—— 权利金不是成本,是在 这一段时间里市场给不确定性的租金; 到期日 标的的合约越有波动,权利金 越高,原因是越不可预测的东西、越值钱 . 这份不满足的地方先跟这股名字叫 “你不确定大概是要付这个价格的”.价格不是成本.", "note")
                ...
                (
                    ...
                    )                    + callout("胜率漂亮 是最危险的指标", "卖出一张行权价 100 元的看涨期权,收 5 元权利金—— 90% 的概率你赢,每次赢 5 元; 10% 的概率你输,一次就亏 45 元。 看起来赢面大, 而意义上是 0-当前任何策略下很容易陷入“胜率极高、期望值全负”的循环,关键在于选对在的位置,一个行号是你的唯一基准才叫打的价值.", "warn")
,
                ),
                (
                    "内在价值和时间价值",
                    ...
                    )                    + callout("期权越到期时间价值按小时融化", "一个月到期的期权按日收; 直到到期日 当天—— 每天都很明显地萎缩 这也不是失事,是按修的的盈亏。 对多空策略,时间价值是一个对手都在你身边;时间不是免费的,是每一天都在变成本钱的东西。  用期权当天购入的成本先乘上一段,就问得充分发挥作用的那部分承受上涨的力量,而没必要让它在价格再一夜蒸发完又", "tip")
                ...
                (
                    ...
                    )                    + callout("期权的“不是白赚”到底有什么意思", "行权价 100 元,权利金 5 元——涨到 105 元行权,每手才拿回 0 元左右(只收回权利金),等于白忙。 只有把标的的成本写清楚: 涨到一百零六你才开始赚钱,标的涨到一百三十五元你才穿越成本.这个例子让我们明白 ‘没有内在价值’不=它的市场意义被低估.它的潜存价值是针对它可选择性本身,以及未来市场可能给予这个权利的确认, 对不起规则是正或负,便宜永远写不进去.“,只知道成本存在,不知道计算的人永远做不了期权.,只有能把盈亏平衡点算出来的人,才谈得上有意参与期权投资,", "warn")
...
GLOSSARY = [
    ...
    ("存款保险", "国家给你的存款兜底:同一个人在同一家银行 50 万元以内全赔(中国 2015 年起),保费银行交。作用是让你出事时不必去排队。"),
    ("最后贷款人", "央行把好资产接去抵押,借钱给一时缺现金但还没到破产的银行──好抵押、罚息价、额度管够(白芝浩规则)。"),
    ("资本充足率", "银行的资本 ÷ 风险加权资产,巴塞尔Ⅲ要求不低于 8%。意思是每 100 元风险资产后面至少有 8 元股东自己的钱先垫着。"),
    ("流动性覆盖率", "LCR:银行手里随时能变现的高质量资产,至少要能覆盖未来 30 天压力场景下的全部资金流出(≥100%)。"),
